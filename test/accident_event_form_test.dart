...
void main() {
  ...
  testWidgets(
    'AccidentEventForm toggles categories, selects injury type, shows attention warning, attaches photos and saves record',
    (tester) async {
      ...
      expect(
        find.byKey(const Key('accident-type-chip-bumpBruise')),
        findsOneWidget,
      );
      ...
      expect(
        find.byKey(const Key('accident-type-chip-foreignIngestion')),
        findsOneWidget,
      );
      expect(
        find.byKey(const Key('accident-attention-card')),
        findsOneWidget,
      ); // foreignIngestion is high risk

      // Enter note
      await tester.enterText(find.byType(TextField), '작은 완구 단추 삼킴 의심, 직후 얼컥거림');
      await tester.pumpAndSettle();
      ...
      expect(
        result?.details,
        contains('비외상 (삼킴/이물/사레) · 이물질 삼킴 · 작은 완구 단추 삼킴 의심, 직후 얼컥거림'),
      );
