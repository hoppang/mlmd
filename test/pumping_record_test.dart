...
void main() {
  group('PumpingRecord domain model tests', () {
    ...
    test('defaults unknown side string to PumpingSide.unknown', () {
      final json = '{"schema":"mlmd.pumping","version":1,"occurredAt":"2026-07-26T10:00:00.000","side":"invalid_side"}';
      final record = PumpingRecord.decode(json);
      expect(record, isNotNull);
      expect(record!.side, PumpingSide.unknown);
    });

    test('buildDetails formats string correctly according to amount and side', () async {
      ...
      final recBoth = PumpingRecord(occurredAt: now, amountMl: 120, side: PumpingSide.both);
      expect(recBoth.buildDetails(locKo), '120mL · 양쪽');
      expect(recBoth.buildDetails(locEn), '120mL · Both');

      final recLeft = PumpingRecord(occurredAt: now, amountMl: 90, side: PumpingSide.left);
      expect(recLeft.buildDetails(locKo), '90mL · 왼쪽');
      expect(recLeft.buildDetails(locEn), '90mL · Left');

      final recAmountOnly = PumpingRecord(occurredAt: now, amountMl: 150, side: PumpingSide.unknown);
      expect(recAmountOnly.buildDetails(locKo), '150mL');
      expect(recAmountOnly.buildDetails(locEn), '150mL');

      final recSideOnly = PumpingRecord(occurredAt: now, side: PumpingSide.right);
    ...
    test('copyWith produces correct modified instance and can clear fields', () {
      final now = DateTime(2026, 7, 26, 10, 0);
