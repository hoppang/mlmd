...
void main() async {
  ...
  if (!await targetDir.exists()) {
    ...
    logger.i('Created directory assets/models');
  }
  ...
  await downloadFile(
    client,
    modelUrl,
    File('assets/models/model_quantized.onnx'),
  );

  client.close();
  logger.i('All downloads finished.');
}

Future<void> downloadFile(HttpClient client, String url, File file) async {
  if (await file.exists() && await file.length() > 0) {
    logger.i('${file.path} already exists. Skipping download.');
    return;
  }

  logger.i('Downloading $url to ${file.path}...');
  try {
    ...
    if (response.statusCode == 200) {
      ...
      logger.i('Downloaded ${file.path} successfully.');
    } else {
      logger.e('Failed to download $url. Status code: ${response.statusCode}');
      if (await file.exists()) {
        await file.delete();
      }
    }
  } catch (e) {
    logger.e('Error downloading ${file.path}: $e');
    if (await file.exists()) {
      try {
        await file.delete();
      } catch (_) {}
