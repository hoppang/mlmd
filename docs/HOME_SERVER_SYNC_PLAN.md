...
- 동시 해소는 새 미해결 충돌로 되돌리지 않는다. 일반 항목은 간단한 알림, 투약 항목은 두 결과·작성자·기기·시각과 최종 적용값을 보여 주는 상세 경고를 남기며, 현재 확인 상태는 기기별 로컬 상태로 저장한다.
- JSON Schema와 golden fixture를 추가하고 Go 통합 테스트에서 계약을 읽도록 연결했다.
- Go test, vet와 build를 로컬에서 검증하고 Linux CI에 race detector를 추가했다.

아직 구현하지 않은 주요 범위는 중요 알림의 가족 구성원별 확인 상태 E2EE 동기화와 실제 운영 백업이다. 현재 동시 해소 알림의 확인은 각 기기에만 저장된다. bootstrap token은 서버 콘솔 설정용 환경 변수이며 최종 관리자 UI를 대체하지 않는다.
...
- 중요 알림은 현재 기기별 확인 상태로 구현되어 있다. 다음 단계에서는 가족 구성원별 확인 상태를 E2EE 변경으로 동기화하고, 확인 전까지 알림함에 유지한다.
...
- SQLite WAL을 안전하게 포함하는 online backup API로 매일 스냅샷을 만든다.
