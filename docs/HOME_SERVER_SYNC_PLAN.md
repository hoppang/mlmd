...
- 실행 중인 WAL 데이터베이스에서 `mlmd-server backup --output <path>`로 일관된 SQLite 스냅샷을 만들 수 있다. 임시 파일에 `VACUUM INTO`를 수행하고 `quick_check`와 디스크 동기화를 통과한 뒤에만 대상 경로에 게시하며 기존 파일은 덮어쓰지 않는다.

아직 구현하지 않은 주요 범위는 백업 파일 암호화, 자동 보관 주기와 복구 명령이다. bootstrap token은 서버 콘솔 설정용 환경 변수이며 최종 관리자 UI를 대체하지 않는다.
